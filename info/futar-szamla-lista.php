...
foreach($_POST as $rend_szamla=>$rend_szamla_value) {
	if(stristr($rend_szamla, 'hidden')) {
		$rend_szamla_id = str_ireplace('hidden', '', $rend_szamla);
		if($rend_szamla_value == 'on') {
			$result_szamla = mysql_query("update rendeles set szamlazhato = 1 where rend_id = $rend_szamla_id", $conn_main);
		}
		else {
			$result_szamla = mysql_query("update rendeles set szamlazhato = 0 where rend_id = $rend_szamla_id", $conn_main);
		}
		unset($_POST[$rend_allapot]);
	}
}

$str_h1 = 'Futárnak elkészített rendelések számlázásra kijelölése';
...
*/
	...
	echo "<table class='normal' cellspacing='0' cellpadding='0'>
		<tr>
			<td class='normal-head'>Mehet</td><td class='normal-head'>ID</td>";
	...
	echo "<td class='normal-head'>Dátum</td>
		...
		$query_rend = "select rend_id, webshop_id, orig_id, rend_datum, rend_ido, nev, irsz, varos, utca, termek_osszeg, szallitas, fizetes, szall_dij, allapot, szamlazhato from rendeles";
		$feltetel ='';
//
/*
		if ($filter_allapot != 0) {
		...
		array_push($str_cond, "(allapot = 4 or allapot = 5)");
...
//			array_push($str_cond, "((rend_id like '%$filter_szo%') or nev like '%$filter_szo%')");
		}
...
*/		
		...
		array_push($str_cond, " futar_ok = '1' and szamla_ok = '0'");
...
//echo $query_rend;
		...
		while (list($rend_id, $webshop_id, $orig_id, $rend_datum, $rend_ido, $nev, $irsz, $varos, $utca, $termek_osszeg, $szallitas, $fizetes, $szall_dij, $allapot_id, $szamlazhato)=mysql_fetch_row($result_rend)) {
			unset($termek_info);
			...
			echo "<tr class='row$sor_tipus'>
					<td class='cell$sor_tipus'>
						<form name='szamla$rend_id' action='".$_SERVER['REQUEST_URI']."' method='post'>
							<input type='hidden' name='hidden$rend_id' value=";
							if($szamlazhato==1) echo "'off'>";
							else echo "'on'>";
							echo "<input type='checkbox' name='szla$rend_id' ";
							if($szamlazhato==1) echo " checked='checked'";
							echo" onchange='document.szamla$rend_id.submit()'>
						</form>
					</td>
