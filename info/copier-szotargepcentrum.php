...
function rendeles_check_szotargepcentrum ($webshop_id, $webshop_host, $webshop_db, $webshop_user, $webshop_pwd) {
	...
	$result_max_id = mysql_query("select max(orig_id) from rendeles where webshop_id ='7' or webshop_id ='3'", $conn_main);
...
function rendeles_download_szotargepcentrum ($orig_id) {

	global $conn_main;

	$result_webshop = mysql_query("select * from webshop where webshop_id = '7'", $conn_main);
...
//////////    RENDELES ALAPADATOK     //////////				
			...
			//futar, utanvet
				if ($kosar_kid == 1 or $kosar_kid == 5) {
					$szallitas = 2;
					$fizetes = 1;
				}
				//futar, eloreutalas
				elseif ($kosar_kid == 2 or $kosar_kid == 6) {
					$szallitas = 2;
					$fizetes = 2;
				}
				//szemelyes atvetel, keszpenz
				elseif ($kosar_kid == 3 or $kosar_kid == 8) {
					$szallitas = 1;
					$fizetes = 3;
				}
				//kulfold, eloreutalas
				elseif ($kosar_kid == 4 or $kosar_kid == 7) {
					$szallitas = 4;
					$fizetes = 2;
				}
...
//				list($kosar_id) = mysql_fetch_row($result_kosar);
				...
				while (list($termekid, $egysegar, $mennyiseg)=mysql_fetch_row($result_termek)) {
					...
					$result_termek_adatok = mysql_query("select nev from $termek_tip where termekid = '$termekid'", $conn);
					list($termek_nev) = mysql_fetch_row($result_termek_adatok);
					
					//marka kideritese
					if ($termek_tip == 'termek_1') {
						$marka = fixEnc2(trim(substr($termek_nev, 0, stripos($termek_nev, " ")+1)));
					...
					$result_termek_insert = mysql_query("insert into termek (rend_id, termekid, termek_nev, marka_id, termek_egysegar, termek_db) values ('$rend_id', '$termekid', '$termek_nev', '$marka_id', '$egysegar', '$mennyiseg')", $conn_main);
