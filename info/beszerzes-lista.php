...
*/
	...
	echo "<td class='normal-head'>Dátum</td>
		...
		$query_rend = "select rendeles.rend_id, webshop_id, orig_id, rend_datum, rend_ido, nev, irsz, varos, utca, termek_osszeg, szallitas, fizetes, szall_dij, allapot from rendeles";
		if ($filter_nagyker != 0) {
//			if (count($str_cond) > 0) {
...
//			array_push($str_cond, "rend_id in (select rend_id from termek where marka_id in (select marka_id from marka where nagyker_id='$filter_nagyker'))");
			...
			$query_rend = $query_rend . " inner join termek on rendeles.rend_id = termek.rend_id inner join marka on termek.marka_id = marka.marka_id";
			array_push($str_cond, "nagyker_id='$filter_nagyker'");
		}
...
//			array_push($str_cond, "((rend_id like '%$filter_szo%') or nev like '%$filter_szo%')");
		...
		$kezd = 0;
		$query_rend = $query_rend.$feltetel." group by rendeles.rend_id order by rendeles.rend_id desc";
...
//		list($numrows) = mysql_fetch_row($result_numrows);
		...
		$numrows = mysql_num_rows($result_rend);
