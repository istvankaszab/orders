...
//		$result_rendeles = mysql_query("select kosar.megrendelt_sorsz, kosar.datum, kosar.ido from kosar where kosar.megrendelt_sorsz > 0 order by kosar.megrendelt_sorsz desc limit $ennyit", $conn);
...
//				$query_rend_insert = "insert into rendeles (webshop_id, orig_id, rend_datum, rend_ido, nev, irsz, varos, utca, telefon, email, szamla_nev, szamla_irsz, szamla_varos, szamla_utca, szallitas, fizetes, termek_osszeg, szall_dij, futar_kod, allapot) values ('$webshop_id', '$orig_id', '$kosar_datum', '$kosar_ido', '$felh_nev', '$felh_irsz[0]', '$varos', '$felh_cim', '$felh_tel', '$felh_email', '$szamla_nev', '$szamla_irsz[0]', '$szamla_varos', '$szamla_cim', '$szallitas', '$fizetes', '$termek_osszeg', '$szallitasi_dij', '$futar_kod', '$allapot')";
//				$result_rend_insert=mysql_query($query_rend_insert, $conn_main);

				newRend($webshop_id, $orig_id, $kosar_datum, $kosar_ido, $user_id, $felh_nev, $felh_irsz[0], $varos, $felh_cim, $felh_tel, $felh_email, $szamla_nev, $szamla_irsz[0], $szamla_varos, $szamla_cim, $szallitas, $fizetes, $termek_osszeg, $szallitasi_dij, $futar_kod, $allapot, '');
...
//////////    ITEM PARAMETERS     //////////				
				...
				//retrieve order id from webshop
...
/*					
					$query_marka =  "select marka_id from marka where upper(marka_nev) like upper('$focsoportnev') or upper(marka_nev) like upper('$csoportnev') or marka_alias like '$focsoportnev' or marka_alias like '$csoportnev'";
					$result_marka = mysql_query($query_marka, $conn_main);
