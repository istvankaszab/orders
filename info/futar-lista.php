...
if($van_csomag > 0) {
	...
	foreach(array_keys($_POST) as $csomag) {
		if(substr($csomag, 0, 5) == 'mehet') {
			$csomag_id = str_ireplace('mehet', '', $csomag);
			$result_csomag = mysql_query("select webshop_id, nev, irsz, varos, utca, telefon, termek_osszeg+szall_dij as utanvet, fizetes, rend_id, orig_id, email from rendeles where rend_id = '$csomag_id'", $conn_main);
			list($webshop_id, $nev, $irsz, $varos, $utca, $telefon, $utanvet, $fizetes, $rend_id, $orig_id, $email)=mysql_fetch_row($result_csomag);
			...
			elseif(isset($_POST['gls'])) {
				print_r($fizetendo.";".$penznem.";".$webshop_nev." ".str_replace(";",",",$orig_id)."/".str_replace(";",",",$rend_id).";;;".str_replace(";",",",$nev).";;".str_replace(";",",",$utca).";;Magyarország;".str_replace(";",",",$irsz).";".str_replace(";",",",$varos).";".str_replace(";",",",$telefon).";".str_replace(";",",",$email).";;".str_replace(";",",",$uzenet).";FDS()FSS()\r\n");
			}
