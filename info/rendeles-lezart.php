...
if(isset($_GET['nagyker'])) {
	...
	$result_allapot = mysql_query("select allapot_id, allapot_nev from allapot where allapot_aktiv = 0 order by allapot_id asc", $conn_main);
	...
	echo "<td class='normal-head'>Dátum</td>
		...
		array_push($str_cond, "allapot in (select allapot_id from allapot where allapot_aktiv = '0')");
